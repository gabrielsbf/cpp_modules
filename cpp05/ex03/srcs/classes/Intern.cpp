...
{
    std::cout << "[INFO]: Instance of Intern is being created | Standard Parameters " << std::endl
              << std::endl;
}
...
Intern::Intern(Intern &src) : form(NULL)
{
    *this->form = src.getForm();
    std::cout << "[INFO]: Instance of Intern is being created | Copy Method " << std::endl
              << std::endl;
}

Intern &Intern::operator=(Intern &copy)
{
    std::cout << "[INFO]: Instance of Intern is being created | Assignment Operator " << std::endl
              << std::endl;
    if (&copy != this)
    {
...
Intern::~Intern(void)
{
    std::cout << "[INFO]: Instance of Intern | is being Destroyed" << std::endl
              << std::endl;
}
...
AForm *Intern::makeForm(std::string form_name, std::string target)
{
    ...
    const FormPair formLists[] = {
        ...
        {"shrubbery", ReturnShrubbery}};

    for (int i = 0; i < 3; i++)
        if (formLists[i].name == form_name)
            return formLists[i].creator(target);
